  ...
  for (let i = 0; i < matchupsTable.children.length; i += 1) {
    ...
    if (tableChild.tagName === "TBODY") {
      for (let j = 0; j < tableChild.children.length; j += 1) {
        ...
        tableRow.children[winPctIndex].style.backgroundColor = redToGreen(
          tableRow.children[winPctIndex].innerText
        );
        // Color actual win column
        if (tableRow.children[wonIndex].innerText === "Yes") {
          tableRow.children[wonIndex].style.backgroundColor =
            "rgba(0,255,0,0.5)";
...
const currentWinProbsGraphData = ({ teams, view }) => {
  ...
  return {
    ...
    datasets: [
      {
        label:
          "Chance of having # of wins through this week, actual wins in red",
        data: winProbsData,
