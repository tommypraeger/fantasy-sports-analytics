...
def fetch_league(league, league_info) -> None:
    ...
    url = (
        f"https://fantasy.espn.com/apis/v3/games/{sport_map[league.sport]}"
    ...
    cookies = {"espn_s2": league.espn_s2}
    ...
    with requests.Session() as session:
        ...
        if resp.status_code == 401 or matchup_resp.status_code == 401:
            raise Exception(
                "The request to access your league was unauthorized. "
    ...
    league.total_matchups = league_json["settings"]["scheduleSettings"][
        "matchupPeriodCount"
    ]
    league.curr_matchups_played = get_curr_matchups_played(
        league, league_json["status"]["currentMatchupPeriod"]
    )
...
def get_score_multipliers(league, matchup_json: dict) -> None:
    ...
    for matchup in matchup_json["schedule"]:
        ...
        if matchup["matchupPeriodId"] in scoring_period_count:
            continue
        scoring_period_count[matchup["matchupPeriodId"]] = len(
    ...
    if len(scoring_period_count.values()) == 0:
        raise Exception(
            f"It looks like the league didn't take place in {league.year}. "
            "Make sure the league ID, year, and sport are correct."
        )
    usual_scoring_period_count = max(
        set(scoring_period_count.values()),
...
def get_matchups(team, league) -> None:
    ...
    for matchup_json in league.schedule:
        ...
        if matchup_json["matchupPeriodId"] > league.total_matchups:
            break
        if (
            matchup_json["away"]["teamId"] == team.id
...
def get_score(
    team, matchup: dict, curr_matchups_played: int, score_multipliers: dict
) -> float or None:
    ...
    if matchup["away"]["teamId"] == team.id:
        score = (
            score_multipliers[matchup["matchupPeriodId"]]
